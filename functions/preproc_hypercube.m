function results = preproc_hypercube(din)
    try
        load(fullfile(din, 'out_gp.mat'), 'results');
    catch
        ...
        ord_concat(idx) = 1e-5 * rand(length(sum(idx)), 1);
        idx = pks_p1_concat < (0.01 * max(pks_p1_concat));
        ord_p1_concat(idx) = 1e-5 * rand(length(sum(idx)), 1);
        idx = pks_p2_concat < (0.01 * max(pks_p2_concat));
        ord_p2_concat(idx) = 1e-5 * rand(length(sum(idx)), 1);
        ...
        [res.pks_pred, ~, ~] = predict(fitrgp([X(:) Y(:)], pks_concat(:), "Standardize",1), [res.X_pred(:), res.Y_pred(:)]);
        [res.ord_pred, ~, ~] = predict(fitrgp([X(:) Y(:)], ord_concat(:), "Standardize",1), [res.X_pred(:), res.Y_pred(:)]);
        [res.pks_p1_pred, ~, ~] = predict(fitrgp([X(:) Y(:)], pks_p1_concat(:), "Standardize",1), [res.X_pred(:), res.Y_pred(:)]);
        [res.pks_p2_pred, ~, ~] = predict(fitrgp([X(:) Y(:)], pks_p2_concat(:), "Standardize",1), [res.X_pred(:), res.Y_pred(:)]);
        [res.ord_p1_pred, ~, ~] = predict(fitrgp([X(:) Y(:)], ord_p1_concat(:), "Standardize",1), [res.X_pred(:), res.Y_pred(:)]);
        [res.ord_p2_pred, ~, ~] = predict(fitrgp([X(:) Y(:)], ord_p2_concat(:), "Standardize",1), [res.X_pred(:), res.Y_pred(:)]);
